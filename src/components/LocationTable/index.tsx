...
function index({ data, aux }: any) {
  return (
    ...
    { data.map(({ code, name }: any, index: number) => (
      <div key={code} className={`flex w-full font-inter mt-4 pb-5 ${index !== data.length - 1 && "border-b-2 border-[#D1D5DB]"} justify-between gap-5 items-center`} >
        ...
        <div className="flex gap-5 items-center">
          ...
          <div className="flex-column">
            ...
            <select className='w-96 py-2 px-3 border rounded-md border-[#D1D5DB] text-[#6B7280] placeholder-gray-400'>
              ...
              { aux.map((name: string) => (
                <option key={Math.random()} value="valor">{name}</option>
              ))}
            </select>
          </div>
        </div>
